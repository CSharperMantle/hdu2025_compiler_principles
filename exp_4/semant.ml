...
let translate (comp_unit : Ast.comp_unit) (ctx : translation_context) :
  ...
  let rec add_const_defs defs ty acc_defs ctx =
    ...
    | d :: ds ->
        ...
        let entry_id, ctx'' = alloc_name_id ctx'' in
        let entry = VarEntry { ty = { elem_ty = ty; dims = dims_vals }; id = entry_id } in
  ...
  let rec translate_comp_unit_item_list items ctx =
    ...
    | item :: rest -> (
        ...
        | Ast.FuncDefItem f ->
            ...
            let entry_id, ctx' = alloc_name_id ctx' in
            let entry = FunEntry { args = arg_types; ret = ret_ty; id = entry_id } in
            let ctx' = { ctx' with names = StringMap.add f.Ast.func_name entry ctx.names } in
            let ctx_body =
              List.fold_left
                (fun ctx (n, t) ->
                  let id, ctx = alloc_name_id ctx in
                  { ctx with names = StringMap.add n (VarEntry { ty = t; id }) ctx.names })
                ctx' named_arg_types
            in
